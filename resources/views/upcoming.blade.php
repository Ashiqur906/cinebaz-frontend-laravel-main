...
@section('content')
    ...
    <section id="home" class="iq-main-slider p-0">
        <div id="home-slider" class="slider m-0 p-0">
            @foreach ($cat_slider as $key => $slider)
                {{-- @dd($key) --}}
                {{-- @if ($key == 0 || $key == 3 || $key == 5 || $key == 7)
            @endif --}}
    ...
    <div class="main-content">
        @foreach ($categories as $key => $list)
            @include('layouts.part.slider_upcoming',['slider' => ['data' => $list->medias, 'name' =>
            $list->title_english,'cat_id' => $list->id,'slug' => $list->slug]])
        @endforeach
